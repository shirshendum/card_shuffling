...
Fisher.yates.times = function(x,n) {
  y = x
  for(i in 1:n){
    y = Fisher.yates(y)
  }
...
## Producing heat map for Over-hand shuffle shuffle 
...
for(j in list.times.shuffle){
  distri.shuffle = replicate(n, Fisher.yates.times(a,j)[k.list])
  distri.theor = replicate(n, sample(a)[k.list])
  par(mfrow=c(3,1))
  for(i in 1:3){
    s = ""
    if(i == 1){
      s = paste0("Histogram for kth position after ",j, " number of Fisher-Yates shuffles.")
    }
...
### for expected counts of fixed point and rising sequence 
n = 10000
a = 1:52
list.of.times = 1:12
...
for(j in list.of.times){
  output.j = replicate(n, countfp(Fisher.yates.times(a,j)))
...
plot(countfp.list,type = "both", xlab = "Number of Shuffles ", ylab = "Expected number of fixed points."
     ,main = "plot of fixed points in Fisher-Yates shuffle")
...
countrs.list = vector()
for(j in list.of.times){
  output.j = replicate(n, countrs(Fisher.yates.times(a,j)))
...
plot(countrs.list,type = "both", xlab = "Number of Shuffles ", ylab = "Expected number of rising sequence."
     ,main = "plot of rising sequence in Fisher-yates shuffle")


output.theoretical.rs = sum(replicate(n, countrs(Fisher.yates.times(a,j))))/n
...
for(j in 1:3 ){
  M[(j+1),] = replicate(n,runs.test(Fisher.yates.times(a,
                                                 list.times.shuffle[j]))$runs)
}

plot(ecdf(M[1,]), col = "RED",main = "Emperical cdf of runs for Fisher-yates",
...
for(j in 1:3 ){
  M[(j+1),] = replicate(n, sum.diff.sign(Fisher.yates.times(a,list.times.shuffle[j])) )
}

plot(ecdf(M[1,]), col = "RED",main = "Emperical cdf of diff sign for Fisher-yates",
     xlab = " Red(Theo),  Black(1),  Green(3),  Blue(7) ",
)
...
for(j in 1:3 ){
  M[(j+1),] = replicate(n, turning.point(Fisher.yates.times(a,list.times.shuffle[j])) )
}

plot(ecdf(M[1,]), col = "RED",main = "Emperical cdf of turning points for Fisher-yates ",
     xlab = " Red(Theo),  Black(1),  Green(3),  Blue(7) ",
)
plot(ecdf(M[2,]), col = "BLACK", add=TRUE)
plot(ecdf(M[3,]), col = "GREEN", add=TRUE)
